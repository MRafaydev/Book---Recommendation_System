import psycopg2
from sql_queries import create_table_queries, drop_table_queries


def create_database():
    """
    - Create & Connect to the booka_data
    - Return the connection & cursoe to books_data

    """
    # connect to the datatbase
    conn = psycopg2.connect(
        host="localhost", database="postgres", user="postgres", password="admin")
    conn.set_session(autocommit=True)
    cur = conn.cursor()

    # create the database with utf8 encoding
    cur.execute("DROP DATABASE IF EXISTS books2_data")
    cur.execute(
        "CREATE DATABASE books2_data WITH ENCODING 'utf8' TEMPLATE template0")

    # close connection to default database
    conn.close()

    # Connect to the bopoks_data database
    conn = psycopg2.connect(
        host="localhost", database="books2_data", user="postgres", password="admin")
    cur = conn.cursor()

    return cur, conn


def drop_tables(cur, conn):
    """
    Drops each table using the queries in `drop_table_queries` list.
    """
    for query in drop_table_queries:
        cur.execute(query)
        conn.commit()


def create_tables(cur, conn):
    """
    Creates each table using the queries in `create_table_queries` list. 
    """
    for query in create_table_queries:
        cur.execute(query)
        conn.commit()


def main():
    """

    - Establishes connection with the sparkify database and gets
    cursor to it.  

    - Drops all the tables.  

    - Creates all tables needed. 

    - Finally, closes the connection. 
    """
    cur, conn = create_database()

    drop_tables(cur, conn)
    create_tables(cur, conn)

    conn.close()


if __name__ == "__main__":
    main()
